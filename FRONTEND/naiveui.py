...
class Ui_Form(object):
    def setupUi(self, Form):
        ...
        self.checkBox_7.stateChanged.connect(self.checkrs)

        self.checkBox_5 = QtGui.QCheckBox(self.groupBox_4)
        self.checkBox_5.setGeometry(QtCore.QRect(30, 20, 151, 21))
        self.checkBox_5.setObjectName(_fromUtf8("checkBox_5"))
        self.checkBox_5.stateChanged.connect(self.checkprob)

        self.retranslateUi(Form)
        QtCore.QMetaObject.connectSlotsByName(Form)

    def retranslateUi(self, Form):
        Form.setWindowTitle(_translate("Form", "NB-Learner", None))
    ...
    def startnb(self):
        ...
        if self.flag:
            ...
            #print self.testdata
            ya=self.clf.predict(np.array(self.testdata).astype(np.float))
            print>>ff,ya
##            if self.pprob:
##                print self.clf.predict_proba(np.array(self.testdata))
            if self.rs==True:
                print self.clf.score(np.array(self.testdata).astype(np.float),self.trainclass)
            if self.pprob==True:
                self.clf.predict_proba(np.array(self.testdata))
